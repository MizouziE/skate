...
document
	...
	.insertAdjacentHTML('afterend', categorySelectorTemplate);
...
const state = {
	...
	get customVariations() {
		return customVariations;
	},
	set customVariations(v) {
		customVariations = v;
	},
	get customVariationsEnabled() {
		return customVariationsEnabled;
	},
...
function getVariation() {
	if (customMode) {
		...
		const valid = customVariations.filter(
			(v) => v.color && v.color.length >= 7
		);
...
function renderHistory() {
	...
	historyEl.innerHTML = sessionHistory
		.map((h, i) => {
			...
			const badge =
				h.status === 'landed'
					? '<span class="pill-status landed">✓</span>'
					: h.status === 'missed'
						? '<span class="pill-status missed">✗</span>'
						: '';
...
function showResult(sector) {
	...
	const displayLabel = variation
		? `${sector.label} — ${variation.label}`
		: sector.label;
