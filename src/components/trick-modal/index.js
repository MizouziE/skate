...
export function applyCustomMode(state, record) {
	...
	state.customVariations = variations.filter((v) =>
		record.variations.includes(v.label)
	);
...
function renderModalPage() {
	...
	document.querySelector('#modalPage').textContent =
		`(${modalPage + 1}/${MODAL_PAGES.length})`;

	document.querySelector('#modalPrev').style.visibility =
		modalPage === 0 ? 'hidden' : 'visible';
	document.querySelector('#modalNext').textContent =
		modalPage === MODAL_PAGES.length - 1 ? 'Save' : 'Next →';
...
export function initTrickModal(state) {
	document
		.querySelector('#customizeBtn')
		.addEventListener('click', () => openModal(state));
	document
		.querySelector('#clearCustom')
		.addEventListener('click', () => handleClearCustom(state));
